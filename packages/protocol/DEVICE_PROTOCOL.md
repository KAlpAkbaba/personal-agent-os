...
**Bounds.** Search ≤ 200 results, ≤ 20 000 directory entries walked, 10 s (every one reported as `truncated: true` with `stop_reason` = `max_results | max_entries | timeout`); read ≤ 65 536 chars per call (`offset`/`length` window the text; `total_chars` is the whole); extract ≤ 65 536 chars of block text (`max_chars` lowers it; the block that crosses the budget is cut and flagged `truncated: true`, the rest dropped, the result says `truncated: true` — never a silent cut); files ≤ 50 MiB for reading and extraction, checked from the directory entry before the file is opened (`unsupported_format`, detail `too_large`); PDF ≤ 200 pages; XLSX ≤ 2 000 rows per sheet (beyond either: `truncated: true`, the counts still true).
...
| `file.inspect` | `{file_id?, path?}` | `{file: record, kind, is_text, encoding?, pages?, sheets?: [name], slides?, lines?, title?, paragraphs?, tables?, too_large?}` (headers only, no text) |
