...
2. **`name` is a plain file name**, sanitised by trimming and otherwise REFUSED rather than rewritten (`validation_error`): ≤ 120 characters; none of the file system's invalid characters (both separators, the drive colon, wildcards, every control character); no `..` anywhere, not `.`; no trailing dot or space; not a reserved device name (`CON`, `NUL`, `COM1`…, with or without an extension); not a secret-bearing name (§6j's list); not an executable extension (§6i's `file.open` list: `.exe .bat .cmd .ps1 .lnk …`); not the companion's own temp prefix. The unique suffix on collision (`bütçe (2).xlsx`) is the companion's, outside the 120.
...
6. **The request** is `GET` of exactly the URL the payload carried — the Cloud Core signs the URL, and the companion adds **no owner token, no cookie, no header of its own**. **Redirects are never followed** (`AllowAutoRedirect=false`): any 3xx is `dependency_unavailable` (not retryable), a redirect onto the same origin included — the URL Cloud Core signed is the URL. Any other non-2xx is `dependency_unavailable` (retryable for 5xx). No client-side decompression: the bytes counted and hashed are the bytes on the wire.
7. **Size, while streaming.** A `Content-Length` that differs from `size` is refused on the headers, unread (`validation_error`). The body is counted as it streams into a temp file (`.pagentos-fetch-<id>.part`, `CreateNew` beside the final name, so a planted reparse point is never opened): one byte past `size` aborts the connection under the server, deletes the partial file and answers `validation_error` — a chunked body that keeps sending cannot make the companion keep writing. A body that ends short is `postcondition_failed`.
8. **Hash, before the rename.** The SHA-256 is computed while streaming and compared with `sha256` BEFORE the temp file gets its final name; a mismatch deletes it and answers `postcondition_failed` (message detail `sha256_mismatch`, never retryable). Nothing with the requested name ever exists unless it has the requested hash.
9. **The final name** is `name`, else `stem (2).ext`, `stem (3).ext`… — moved with `overwrite: false`, so an existing entry of any kind (a file, a directory, a reparse point squatting the name) is never replaced or opened, only skipped. What is reported is the path the file system resolved for the kept file, checked to lie inside the Downloads root once more; the record is §6j's (`sha256` present under 8 MiB), the `file_id` is issued and resolves again.
