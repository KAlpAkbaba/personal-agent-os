...
class HealBody(BaseModel):
    """The same workspace/port contract as POST /v1/selfhealing/pipeline/run: the loop runs
    the REAL pipeline on a staging and a production workspace under the self-healing root."""
...
async def heal_opportunity(
    ...
    failing gate in the reason. Owner-gated like every evolution route."""
    from app.evolution.closed_loop import ClosedLoop
    from app.selfhealing.errors import SelfHealingError
    from app.selfhealing.routes import build_pipeline

    runtime = _runtime(request)
    selfhealing = request.app.state.selfhealing
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail={"error_class": str(exc.error_class), "message": exc.message}
        ) from exc
