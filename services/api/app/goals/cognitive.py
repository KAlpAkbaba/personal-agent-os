...
from app.uistate import UiState, UiStateEvent, UiStatePublisher, get_publisher
...
class Orchestrator:
    ...
    def _publish(self, goal: Goal, state: UiState, *, detail: dict[str, Any] | None = None) -> None:
        """Publish through the ADR-0052 contract: state, identity and bounded numbers.

        The renderer draws states, never internals, so the goal's own detail is reduced to
        the numeric/short-token metadata the contract allows; anything content-shaped is
        dropped at the publisher's boundary anyway.
        """
        self.ui_publisher.publish(
            UiStateEvent(
                ...
                subsystem="goal",
                goal_id=str(goal.goal_id),
                label=str(getattr(goal, "title", "") or "")[:64] or None,
                status=str(getattr(goal, "status", "") or "") or None,
                at=self._now(),
                metadata={
                    key: value
    ...
    def run(self, goal_id: uuid.UUID) -> LoopResult:
        ...
        try:
            ...
            self._publish(goal, UiState.ERROR, detail={"error_class": type(exc).__name__})
            self._ledger(
                goal,
