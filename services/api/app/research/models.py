...
#: order (the flaky ``test_voice_research_followup`` binding, 2026-09-08). The generator
#: itself is :func:`app.ids.focus_row_id`, shared with ``app.operator.models``'s object
#: focus, which reads its stack exactly the same way; the reasoning is in that module.
class ResearchFocusRow(Base):
    ...
    #: Time-ordered on purpose (see ``app.ids.focus_row_id``): the stack's tiebreak key.
