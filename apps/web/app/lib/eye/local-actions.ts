 ...
 *
 * The relayed object is exactly §5.1's shape and nothing more:
 *
 * ```
 * {local: {state, running, camera_label, error_class, observed_at}}
 * ```
 *
 * `changed` (the store's own idempotency flag) is deliberately NOT relayed:
 * the server decides `verified` vs `already` from its own read-back and its
 * own turn bookkeeping, not from a client claim. The 8 s bound lives in the
 * store (`LOCAL_EYE_TIMEOUT_MS`), so `run` settles within it by construction
 * and never rejects.
 */
