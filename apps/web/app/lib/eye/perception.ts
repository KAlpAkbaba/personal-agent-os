 ...
 *
 * `BrowserFrameSource.capture()` is the only function in this entire module
 ...
 * once, synchronously, inside `PerceptionSession#tick()`, by
 * `computeGridLuminance` — which immediately reduces it to a ~108-number
 * luminance grid and discards the pixel buffer (nothing holds a reference to
 * it once `tick()`'s local `pixels` variable goes out of scope at the end of
 * the synchronous block that reads it). From there on on, `tick()` only ever
...
/** One captured frame, reduced to nothing but plain numbers before use. */
...
export interface FrameSource {
  ...
  /** One frame, reduced to raw pixels — or `null` if not ready yet. */
...
export class BrowserFrameSource implements FrameSource {
  ...
  capture(): CapturedFrame | null {
    ...
    // `data` is consumed synchronously by the caller (see PerceptionSession#tick)
    // and never stored past this call.
...
export class PerceptionSession {
  ...
  async #tick(generation: number): Promise<void> {
    ...
    const frame = this.#frameSource.capture();
    if (!frame) {
    ...
    const grid = computeGridLuminance(frame.data, frame.width, frame.height);
