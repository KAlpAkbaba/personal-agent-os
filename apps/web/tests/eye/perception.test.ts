...
import { PerceptionSession, type CapturedFrame, type FrameSource } from "../../app/lib/eye/perception";
...
class FakeFrameSource implements FrameSource {
  ...
  private frame: CapturedFrame | null = {
    data: new Uint8Array(4 * 4 * 4), // a tiny, uniform, synthetic "frame" — never real imagery
    width: 4,
    height: 4,
  };
...
describe("stop() is immediate and total", () => {
  ...
  it("a sample cannot be posted at all once stop() has run before the pre-post check — even reentrantly, mid-tick", async () => {
    // capture() runs synchronously inside tick(), before the pre-post guard. A
    // frame source that calls stop() from inside capture() proves the guard
