...
describe("end to end: a provider function call opens and closes THIS device's camera through the one EyeStore", () => {
  it("eye.enable then eye.disable: camera first, relay with the real observation, reason voice:<utterance>", async () => {
    ...
    expect((t.relays()[0].body as { arguments: unknown }).arguments).toEqual({
      utterance: "Gözünü aç.",
      observed_after: { local: { state: "ACTIVE", running: true, camera_label: "Integrated Webcam", error_class: null, observed_at: AT } },
    });
    ...
    expect((t.relays()[1].body as { arguments: { observed_after: { local: { state: string } } } }).arguments.observed_after.local.state).toBe("ACTIVE");
...
describe("the shapes", () => {
  it("observed_after is exactly {local: {state, running, camera_label, error_class, observed_at}} — `changed` never travels", () => {
    expect(
      observedAfter({ state: "ACTIVE", running: true, camera_label: "cam", error_class: null, observed_at: AT, changed: false }),
    ).toEqual({ local: { state: "ACTIVE", running: true, camera_label: "cam", error_class: null, observed_at: AT } });
  });
