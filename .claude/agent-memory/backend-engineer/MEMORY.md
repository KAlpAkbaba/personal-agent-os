...
- [B11 WebPush status](project_b11_webpush_status.md) — completed 2026-09-17 on branch worktree-agent-a4cce9e70a6f6d4db @ 9b7e3ef; 117 new tests, full suite 11764 passed, 5 mutation proofs; not merged/pushed
- [Health check needs a status field](feedback_health_check_status_field_required.md) — a checks[name] dict with no "status" key silently marks the WHOLE process degraded (app.health.is_degraded); use "ok" for an unconfigured-but-optional provider
- [Owner-facing HTTPException guard](feedback_owner_facing_http_exception_guard.md) — an AST test fails any route whose HTTPException(detail=...) mentions the caught exception name; use app.errors.owner.log_and_detail instead of str(exc)/f"{exc}"
