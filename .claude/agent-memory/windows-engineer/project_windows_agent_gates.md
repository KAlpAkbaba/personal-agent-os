...
- `.GetNewClosure()` on a scriptblock COPIES the `$script:` variables it captures into the closure's own module scope. A later `$script:x = ...` in the outer script is invisible to it. For a fake that must change its answer between calls (a fake Cloud Core, a call counter), capture a **hashtable** instead and mutate its entries — the reference is shared. This silently made a qualification's fake device row answer with the pre-update version.
- `BrowserWorkerHostTests.A_worker_that_stops_answering_pings_is_killed_and_replaced` is a known flake under full-suite parallel load (a ping-timeout race); it passes in isolation. Rerun before treating it as a regression.
- `scripts/tests/agent-update.tests.ps1` and `scripts/qualify-staged-update.ps1` are gates now (`quality-gate.ps1` and `.github/workflows/ci.yml`). The qualification needs `dotnet build -c Release` first; it resolves the DeviceService from `src/PagentOS.DeviceService/bin/Release/net10.0-windows/` and never reads the installed one.
