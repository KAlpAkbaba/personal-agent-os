...
Both times the SAME repo has a sibling module (M3: none readily available at review time; M8: `remediation.py::_authorized_file`) that does the correct thing — resolve the full path first, THEN check ancestor containment (`root not in path.resolve().parents`) — proving the fix is well-understood in this codebase, just not applied consistently.
