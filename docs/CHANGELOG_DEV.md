...
- Honest proof boundary: the mobile acceptance bullets are proven against the real API through the reference client and integration tests, which establishes the server contract. Push arriving on a locked phone and microphone/realtime capture under iOS/Android backgrounding need a physical device and stay batched as owner actions.
