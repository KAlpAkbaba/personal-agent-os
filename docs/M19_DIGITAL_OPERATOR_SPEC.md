...
| `pointer.stream_begin` / `pointer.stream` / `pointer.stream_end` (ADR-0199) | `{session, window_id?}` / `{session, frames}` / `{session}` — the owner's hand as the mouse; the batch normally arrives as the device-protocol `pointer_stream` frame and crosses the pipe one-way | `{session, window_id, opened_at, replaced?}` / `{applied, dropped, reason?}` / `{ended, ended_by, moves, buttons, dropped, duration_ms, released[]}` — NOT guarded: refused whole when no stream is open, the session is locked or the shell's window is in front; moves clamped ±200/frame, ≤ 60 frames/s; a held button never survives the stream (DEVICE_PROTOCOL.md §5b/§6r) |
