...
| 474 | Android project generation | `counter-mobile` gerçek bir Gradle Kotlin DSL projesi üretir (kök settings/build, `app` modülü, AndroidManifest, string kaynağı, MainActivity, saf `Counter` ve JVM birim testi); masaüstü şablonuyla aynı yuva disiplini, uzantı izin listesi ve M23 politikası; paket kimliği sluga göre türetilir (rakamla başlamaz, ayrılmış sözcük olmaz), versionCode her sürümde büyür, başlık strings.xml için kaçışlanır; Gradle sarmalayıcısı bilinçli olarak YOK (betik render edilmez) | Üretilir | DONE | PA | P3 | 425 | B49 | app/nativefactory/generator.py:android_package/android_version_code/android_string_resource; templates/counter-mobile/ | test_nativefactory_b49.py (XML olarak ayrıştırılan manifest ve kaynak; Gradle kimlikleri; paket bildirimleri; politika; diske üretim; JDK'sız derleme reddi) | derleme READY_FOR_OWNER (475) | no | v1.0'ı bloklamaz |
| 475 | APK | Proje üretilir (474); derlemek JDK 17 + Gradle ister: bu makinede SDK platform 32/33 ve build-tools var, JDK/Gradle/cmdline-tools YOK; indirme sahip izni ister | Üretilir | BLOCKED_PROVIDER | NYP | P3 | 474 | B49 | app/nativefactory/ | stacks testleri (JDK'sız dürüst ret) | JDK kurulumu READY_FOR_OWNER | SDK kurulumu (JDK indirme izni) | Madde 33 |
| 476 | AAB | 475 ile aynı: `bundleRelease` JDK + Gradle ister; manifest `bundle` yolunu ve versionCode'u taşır | Üretilir | BLOCKED_PROVIDER | NYP | P3 | 475 | B49 | app/nativefactory/ | — | JDK kurulumu READY_FOR_OWNER | SDK kurulumu (JDK indirme izni) | — |
| 477 | Android emulator test | Emülatör ve sistem imajları var; AVD ve cmdline-tools yok, APK (475) yok | Koşar | BLOCKED_PROVIDER | NYP | P3 | 475 | B49 | app/nativefactory/ | — | AVD + APK READY_FOR_OWNER | SDK kurulumu | — |
| 478 | Android device test | Fiziksel cihaz gerekir | Koşar | BLOCKED_PROVIDER | NYP | P3 | 477 | B49 | app/nativefactory/ | — | — | fiziksel cihaz | — |
