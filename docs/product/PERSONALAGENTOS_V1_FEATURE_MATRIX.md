...
| 379 | Quiet hours | Yok | Sessiz saatler | DONE | PA | P1 | 378 | B11 | app/notifications/models.py, app/notifications/service.py | test_notifications.py | üretim turu bekliyor (Karar 0) | no | Gürültü hakkında, gizleme hakkında değil: ertelenen satır kutuda duruyor — 2026-09-16: pencere UTC saatine göre ölçülüyordu (İstanbul'da gece 02:00–10:30 oluyordu); artık Europe/Istanbul duvar saatinde, regresyon test_quiet_hours_are_the_owner_s_night_not_utc_s (RED kanıtlı) |
...
| 744 | Ambiguity clarification | İki alanı adlandıran cümleye hangisi, tek alana ne yapılacağı sorulur; yönlendirilmiş cümle asla sormaz; soru tur kaydına yazılır, sesli söylenmesi ayrı bayrakla (`voice_clarify_aloud_enabled`, kapalı) çünkü modelin kendi cevabıyla üst üste binebilir | Soru sorar | PARTIAL | PA | P2 | 743 | B51 | app/voice/intent_router.py:clarification_for; service.py SB_SAY | test_intent_router_b51.py (iki alan; tek alan; sessizlik; bayrak) | sesli sorunun gerçek oturumda çakışmadığı PROVEN_REAL değil | no | varsayılan: kayıtta, sessiz |
| 745 | Reference resolution | 'bunu/şunu' son 30 dakikadaki odak nesnesine çözülür; 'bu hafta' gibi zaman ifadesi referans sayılmaz, eski odak 'bu' değildir; sonuç tur kaydında `deictic_reference` | 'bunu/şunu' çözülür | PARTIAL | PA | P2 | 49 | B51 | app/voice/intent_router.py:resolve_deictic_reference; app/operator/focus.py | test_intent_router_b51.py (taze odak; zaman; bayat odak; işaret yok) | çözülen referansı henüz hiçbir araç okumuyor | no | 49 ile aynı iş |
| 746 | Turkish paraphrases | Kural tablosu genişletilmedi (741 daraltma disiplini); kuralların kaçırdığı başka söyleyiş bayrakla modele gider, belirsizse soru üretir; 91/91 beklenen yönlendirme kurallarla tutuyor | Geniş | PARTIAL | PA | P2 | — | B51 | app/voice/intent_router.py; app/voice/intents.py | test_intent_router_b51.py · test_intent_misroutes.py | geniş söyleyiş kapsaması model bütçesine bağlı | no | — |
| 747 | ASR-noise variants | Yönlendirme setinin her cümlesi transkripsiyon bozulmalarıyla (kesme işaretsiz, başta dolgu, noktalı i) yeniden koşulur: 150 varyant, 0 yanlış yönlendirme, 6 kayıp yönlendirme; kayıpların hepsi noktalı i (ı→i): turkish_casefold ı ile i'yi bilerek ayırır, katlamak 741'i değiştirirdi — kayıplar testte adıyla sabitlendi, yenisi de düzeltileni de testi kırar | Dayanıklı | PARTIAL | PA | P2 | 746 | B51 | tests/unit/test_intent_router_b51.py:_asr_variants | test_intent_router_b51.py (asla yanlış yönlenmez; aslı nereye gidiyorsa oraya) | noktalı i kayıpları (kanıtta ve KNOWN_DOTTED_I_LOSSES listesinde) | no | — |
| 748 | Routing corpus expansion | Varyantlar her ROUTING_SET vakasından otomatik türetilir: sete eklenen her cümle kendi ASR varyantlarıyla birlikte denetlenir (107 vaka, 150 varyant) | Sürekli büyür | PARTIAL | PA | P2 | 739 | B51 | tests/voice_corpus/routing.py; tests/unit/test_intent_router_b51.py | test_intent_router_b51.py | — | no | — |
