...
| I | 31 — the Tailscale SSH check to `pagentos-core` expired (2026-09-08 02:44Z: "To authenticate, visit https://login.tailscale.com/a/…"): run `ssh root@pagentos-core exit` in a terminal and complete the check in the browser; the M21 Cloud Core release (571ddb2) is armed from a frozen checkout with `scripts/core/qualify-m18-4.ps1 -WaitForHost -WaitForHostMinutes 240` and ships unattended within a minute of the check (if the 4 h window lapses, re-run that command from the repo root) | `READY_FOR_OWNER` (a browser login) | 1 min |
