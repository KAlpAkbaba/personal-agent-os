...
**The rule this milestone paid for three times.** Drive the two halves of a contract against each other with something neither wrote. M24's vacuous verification gate, M25's non-functional write path and M25's unreadable UI vocabulary were all invisible to full, passing suites, because in each case the fixture answered the way the code hoped: a fake device that never read the command, a lab that never called the service, a web test that fed the web its own words. Three permanent guards now exist for that class — the fake device reads the manifest, the service-command lab runs the Cloud Core's own argv against the real editor, and the vocabulary test reads the other side's source file. Between them they found four defects in one afternoon.
