...
**Addendum 4 — the gauge measured its neighbours, so the guarantee was moved to fit the instrument; both are corrected (2026-09-08).** `BoundedReadTests` failed about one run in four on the addendum 3 bound above — `allocated 16,4 MiB` against `< 16` for the 6 MiB CSV read — and the first response to it (`795e860`) took the best of six attempts instead of three and moved the bound to 24 MiB. That is the wrong half to change: 24 MiB was chosen to cover the noise, not to describe the read, and a bound relaxed to fit its instrument stops being a guarantee. Both are now corrected — the bound is back at 16 and the measurement is what changed.

**Root cause.** `BombFixtures.Measure` reads `GC.GetTotalAllocatedBytes(precise: true)` and `Process.WorkingSet64`. Both are PROCESS-wide, xunit runs collections in parallel, and this assembly reached 758 tests, so whatever a neighbouring collection allocated inside the measured window was charged to the read under test. The read's own cost is 12–13 MiB (4 MiB of bytes and 8 MiB of UTF-16, measured), which leaves under 4 MiB of headroom for the rest of a busy process — and about one run in four it was not enough. Independent of M25: it reproduces with the scenes lab excluded entirely.

**Why the obvious fix is worse than the bug.** `GC.GetAllocatedBytesForCurrentThread` is per-thread and would cost nothing, but `DocumentCapabilities.ExecuteAsync` hands every read-only name to `Task.Run`, so the work does not run on the measuring thread: the counter would see almost none of the read and would report a bound as KEPT when it was broken. A gauge that fails loudly under noise is worth more than one that passes silently through a real regression, so the measurement had to be made where nothing else was running instead.

**The mechanism (`Support/TestActivity.cs`).** An assembly-level `BeforeAfterTestAttribute` registers every test body in the run — xunit takes the attribute from the assembly as well as the class and method — so a gauge can ask for a window with nothing else in it: new tests wait at the door, the ones already running are allowed to finish, and the register is read back afterwards to confirm the window really was undisturbed rather than assumed to be. The gauge then rests on which of two facts it holds: the allocated-bytes counter can only ADD, so **a reading under the bound is conclusive however busy the process was** and is returned as it stands; a reading OVER the bound proves nothing at all and is measured again behind the door — first with the door merely shut, then, if that still will not settle it, after a full drain. Over the bound in a verified quiet window is this work's own cost, and the caller is right to fail on it. Every wait is bounded: an unverified window yields a number the gauge reports and says it could not verify, never a suite that hangs.

**What it costs.** Nothing on the normal path, by design. Measured across full parallel runs, the four gauged tests read 13.4 / 12.4 / 7.0 / 0.6 MiB against bounds of 16 / 16 / 64 / 64 and never left the fast path; the door is shut only when noise actually strikes. The suite went from 41 s to 42 s, and six consecutive runs are green at the restored bounds.

**The regression facts (`Documents/AllocationGaugeTests.cs`).** A neighbour that registers exactly as a test body does and allocates hard: the gauge reads +101 MiB beside it and +1.0 MiB in the quiet window, for work that allocates 1 MiB. Beside it, two facts that hold the mechanism up rather than the number — that the assembly registrar is applied at all (were it silently dropped, the register would read empty whoever was running, every drain would return at once, and every quiet window would merely LOOK quiet), and that a reading already under its bound does NOT close the door, since a gauge that stopped the suite on every gauged test would be paid for by every run.

**A second flake, found and fixed in the same pass, with two races in it rather than one.** `BrowserWorkerHostTests.A_worker_that_stops_answering_pings_is_killed_and_replaced` gives its host a 100 ms ping interval and a worker that answers no ping — and that one number sets BOTH how long the first kill takes (three missed pongs, so four ticks) and how long the REPLACEMENT lives before the same guard kills it in its turn. The replacement therefore had about 400 ms in which to be started, serve a request and be asserted on. The first race was the closing `Assert.True(host.WorkerRunning)`, a condition true only inside that window while nothing bounded how long the assertions before it took; proven by inserting a 600 ms delay, after which it fails every time. The second surfaced only under a Release run beside the rest of the suite, and was worse: the ECHO REQUEST ITSELF failed with `dependency_unavailable` — "killed by the companion (missed 3 consecutive pings) while this request was in flight". Both are the same cause, so both are fixed at it. The interval moves to 500 ms, which leaves the first kill at two seconds (far inside the test's ten-second waits) and gives the request four times the margin it needs on a slow runner; and the sampled `WorkerRunning` becomes a wait for the SECOND liveness kill, which is deterministic and the stronger claim, since only a worker that was alive and being pinged can be killed for missing pings. `Starts == 2` is re-asserted after it: on a host that is not eager, only a request starts a worker.

**The rule.** When a test's instrument is process-wide and the process is parallel, the number it reports is not the number it names. Fix the instrument; the guarantee is not the part that moves.
...
## ADR-0116 — Two names for one action, and the incident that pointed at neither (2026-09-11)
