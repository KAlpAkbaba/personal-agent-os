...
**The rule this milestone paid for twice.** Ask the reviewer to prove the CENTRAL claim against something the code has never seen. M24's vacuous verification gate and M25's non-functional write path were both invisible to full, passing suites, because in each case the suite's fixture answered the way the code hoped. The lab that closes this addendum exists for exactly that reason, and it found three defects in its first run.
