...
the shortlist) — the fix is ADR-0074 (shortlist refill, budget-aware waves, a truthful thin
result).
...
| 13.3 | Diagnostics stay available and are spoken only on request: `Teknik anlat.`, `Hangi sayfalar elendi?`, `Araştırma sırasında ne sorun oldu?` | `NOT_YET_PROVEN` | API: the technical level / the diagnostics question kinds return `ResearchDiagnostics` with evidence refs; the executive level never does. Real: NOT observed in the run of 2026-09-06 (the session closed by `page_unload` 30 s after the result; no technical intent on the fresh session). Its own check: `scripts/core/owner-m18-2-followup.ps1` — a new session, `Teknik anlat.`, proven from the record: the router's technical intent + `activity.explain` at the technical level, diagnostics only now, the completed task's `ready_at`/artifact unchanged, no `research.start`, no new task, no release, ≤ the technical speech budget. |
...
| 14.5 | A wake alarm is created by voice in the owner's timezone (`Yarın sabah 07:30'da beni uyandır.`, `Her hafta içi 07:15'te …`, `Saat 08:00'e alarm kur.`, `90 saniye sonra test alarmı kur.`), acknowledged only from its receipt, and `Sabah alarmım kaçta?` is answered from the record; `Alarmı iptal et.` cancels it | `NOT_YET_PROVEN` | API: `resolve_intent` pins every phrase; `tr_time` in Europe/Istanbul (DST-safe, never UTC); `alarm.create/cancel/status` receipts and speech. Real: B — the test alarm spoken, its `alarm.created` ledger row and receipt on the session. |
| 14.6 | The alarm fires without a voice session, a focused tab or a microphone: the clock fires an ARMED alarm at its instant; a restart or reconnect never fires it twice; the device's own armed fallback rings once when the cloud does not reach it | `NOT_YET_PROVEN` | API: the clock fires after a fresh process's first tick; two ticks / two processes / a reconnect = one firing. Windows: arm → consume / fallback / expire on a manual clock, once across a restart. Real: B — the firing is a routine firing at the scheduled instant (the record's `triggered_at`), the device audit shows the arm consumed. |
...
| 14.14 | Automatic display-off is conservative and owner-configurable: only sustained AWAY (`away_after_s`) or sustained, confident LIKELY_ASLEEP (`asleep_after_s`, `asleep_min_confidence`) with `auto_off_enabled`; UNKNOWN, stale, eye disabled, low confidence, an alarm context, a holdoff or a display already off never turn a display off; `LIKELY_ASLEEP` never comes from one frame; every decision that acts is a receipt and a ledger row | `NOT_YET_PROVEN` | API: the decision table line by line; the engine's sustain rules (unchanged). Real: the separate longer observation after C, when the owner wants it. |
| 14.15 | The owner's ambient commands persist with receipts: `Uyurken ekranları kapat.`, `Ben yokken ekranları kapat.`, `Otomatik ekran kapatmayı aç/kapat.`, `Ben geri geldiğimde ekranı aç.` | `NOT_YET_PROVEN` | API: intents pinned; `ambient.set_policy` receipts; `ambient.policy_changed` ledger rows. Real: B or C — one spoken policy change on the record. |
| 14.16 | At alarm time the displays wake before or with the audio, and a failed display wake never suppresses the alarm audio; the wake works with the eye disabled | `NOT_YET_PROVEN` | API: the sequence with a failed `display.wake` port still reaches the audio step; no eye/presence dependency (structural). Real: B — the `display.wake` receipt precedes `media.play`/`alarm.start`. |
