...
| # | Criterion | Status | Evidence required |
|---|---|---|---|
| 14.1 | `/core` in Minimal mode is a full-viewport Living Core: the Core covers 60–80 % of the usable viewport by aspect ratio, near-black ground, no page scroll, controls as fading overlays, fullscreen only on the owner's gesture with a visible exit, a PWA manifest; the old small wireframe is gone | `NOT_YET_PROVEN` | Web: `stageSizeFor` across aspect ratios; the overlay fade reducer; the fullscreen handler bound to a button only. Real: A — the owner sees it on their own screen. |
| 14.2 | The gold/amber identity is layered and dense — outer field, containment shell, topology shell, orbital layers, data/circuit layer, particle transport, processor structures, energy chamber, nucleus — an original PersonalAgentOS language, not a film asset | `NOT_YET_PROVEN` | Web: the scene mounts each layer at its tier; `docs/M18_3_LIVING_CORE_VISUAL_IDENTITY.md`. Real: A — scale, depth, colour reviewed by the owner. |
| 14.3 | Real state drives the Living Core, unchanged truth rule: idle breath only; listening pulls inward; owner speech from the measured level; thinking/tool/research/memory/eye/waiting/shadow as published; SPEAKING semantic with RMS on amplitude only; NEW `alarm.*` wake surge and `display.*` on the ambient strip; contract v3 both sides | `NOT_YET_PROVEN` | Web: every new state its own visual and zero channels otherwise; alarm never displaces thinking/speaking; display never reaches the geometry; a v2 server still renders. Real: A — speaking, listening, eye and the research constellation watched live. |
| 14.4 | High / Balanced / Low stay bounded, a hidden tab draws nothing, reduced motion is still, no WebGL → the 2D view in the same identity; cognition never depends on the renderer | `NOT_YET_PROVEN` | Web: `sceneBudgetFor` equals the mounted objects; the hidden/still paths. Real: A — smooth on the owner's GPU at the chosen tier. |
...
| 14.7 | The named YouTube item really plays in the worker's dedicated `alarm` profile, verified by the media element (`currentTime` advancing, not paused); a challenge / bot check / consent wall / autoplay block / missing element is a truthful failed receipt and the tone with its ramp — never a bypass, never a click, never the owner's own Chrome | `NOT_YET_PROVEN` | Browser: each failure reason; the profile path rule; no test launches a browser. API: media failure → tone fallback with the failure receipt. Real: B — `media.play` verified with the owner's URL (or the truthful fallback, named). Absorbs M18 row 12.13. |
| 14.8 | The wake volume ramps from a low start to the owner's target over 10–30 s and never touches the Windows master volume; nothing is left changed after the alarm | `NOT_YET_PROVEN` | Browser: the in-page ramp script; Windows: structural — no endpoint/session volume API in the companion; API: the ramp figures on the receipt. Real: B — the owner hears the rise; the receipt carries start/end/ramp. |
