...
| 9.2 | Hetzner → Tailscale → DeviceService → Companion → worker → real Chrome: `browser.session_open` + `browser.fetch_evidence` of a public page succeed with metadata | `PROVEN_PROXY` (2026-09-03 local real chain: dev Cloud Core → dev agent → worker → real Chrome; the Hetzner/Tailscale leg is not) | Command rows with `command_id`/`trace_id`; companion audit; Chrome window observed. |
