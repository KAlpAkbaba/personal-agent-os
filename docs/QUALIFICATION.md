...
| 14.11 | `display.off`, `display.wake`, `display.status` are governed capabilities on the DeviceService / SessionCompanion path with receipts (action_id, requested_state, reason, terminal_status, observed_after, evidence_refs, error_class); direct owner commands (`Ekranları kapat.` / `Ekranları aç.`) use the normal action authority and are spoken only after read-back | `NOT_YET_PROVEN` | API: receipt tests per outcome (verified / already / refused / failed). Windows: the observer's state read back after each request. Real: C — the `display.off` receipt and the `display.on` observation on the heartbeat. Absorbs M18 row 12.19's positive half. |
