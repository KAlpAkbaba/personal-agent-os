...
| 9.13 | Browser lifecycle invariant on the owner machine: one research job = one worker + one PagentOS-profile Chrome process + one window; every command proves reuse (`session_uid`, `browser_pid`, bounded `tab_count`); clean exit leaves no PagentOS-profile Chrome | `NOT_YET_PROVEN` | OWNER_ACTIONS item 13: `real-browser-smoke.ps1 -Mode lifecycle`. **Root cause of the 2026-09-03 window cascade (proven by experiment, not inferred):** a second Playwright launch on the dedicated profile while a PagentOS Chrome already holds it fails within 0.1 s AND opens a new window in that Chrome; a worker killed by the companion/installer cannot close its Chrome (orphan holding the profile lock), and every later launch attempt (Cloud Core retries per URL/query, companion worker restarts, reruns) added one window. `chrome.exe --version` in browser detection was tested and did NOT open a window while the owner's Chrome was running. Fix: single owned research browser with orphan reaping, `browser_lifecycle_violation` instead of retried launches, bounded tabs, companion/installer cleanup of profile-bound Chrome only, companion file log. |
