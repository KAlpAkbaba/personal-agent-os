...
| 12.26 | A spoken mutation is acknowledged only from a verified receipt: `Gözünü kapat` → the real capability executes, the runtime is read back, and the assistant says `Gözümü kapattım efendim.` (or `zaten kapalı`, or `kapatamadım; işlem doğrulanmadı`) — never *"öyle olmuş gibi düşün"* | `NOT_YET_PROVEN` | Contract: `docs/M18_ACTION_CONTRACT.md` §1, §5 (ADR-0063). API: `test_voice_eye_tools.py` (verified / already / unverified / safety-net-same-turn), `test_actions_receipt.py` (the speech table contains no banned phrase). Real: `owner-m18-eye.ps1` `eye.disable_receipt_verified`, `eye.confirmed_only_after_ack` (the session's `first_audio` after its `tool_done`). The owner's second real attempt (2026-09-06) is the defect evidence: the eye closed, the assistant made it up. |
| 12.27 | The symmetric enable path exists and is stateful: `Gözünü aç` / `Kamerayı aç` / `Beni tekrar izle` reopen the real local camera through the same contract; DISABLED→ENABLING→ACTIVE→DISABLING→DISABLED, idempotent, never two streams or two loops | `NOT_YET_PROVEN` | Web: `tests/eye/store.test.ts` (enable → ACTIVE → disable → DISABLED → enable → ACTIVE with exactly two camera opens; join-in-flight; counters {1,1,1}). API: `test_voice_intents.py` (every phrase → `EYE_ENABLE`), `test_voice_eye_tools.py` (enable verified only when the client observed `ACTIVE`; permission_denied spoken as such). Real: `eye.enable_receipt_verified`, `eye.reenabled_read_back`. The owner's second attempt: `Gözünü aç` did nothing. |
