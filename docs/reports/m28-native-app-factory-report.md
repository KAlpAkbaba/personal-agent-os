...
## PROVEN_REAL: the Windows pipeline (`docs/evidence/m28-native-windows-lab-2026-09-09-102001.json`)

> **Corrected 2026-09-11.** This heading previously cited `…-091306.json`, which does not exist in the repository. The eight real lab runs are `…-102001`, `…-131331`, `…-132200`, `…-141704`, `…-141913`, `…-142019`, `…-160514`, `…-160738`.
...
| `dotnet build` | exit 0 |
| `dotnet test` | **6/6**, including persistence across a restart — corrected 2026-09-11; every evidence file records 6, not 5 |
...
| ~~Voice intents + the `nativeapps` corpus category~~ | **done.** 1813 cases across 21 categories at merge, `nativeapps` 94 (recounted 2026-09-11: 1889 / 22), every earlier category unchanged case for case; 78 voice tests across two files. Stage 26 rows 26.10-26.12. |
...
> **Correction (2026-09-11, ADR-0098).** The paragraph above is wrong and was superseded the same day it was written. `file.open` refuses executables **by name** — an executable extension is a `security_scope_error` even inside an authorised root — so it can never start a built application. The real grant is ADR-0098: `app.launch` resolves a path under the native root by resolve-then-contain (`OperatorCapabilities.cs:1346`, rule isolated at `:1371`, eight tests in `NativeLaunchTests.cs`). That is what the item-28 qualification actually used: step 35 of `docs/evidence/item28-unlocked-20260909-190753.json` is `app.launch`, not `file.open`.
