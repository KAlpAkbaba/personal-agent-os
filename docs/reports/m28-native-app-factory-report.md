...
| **`app.launch` cannot reach a built application** | `app.launch` allowlists "a name, or an absolute path under Program Files / Windows" (`OperatorCapabilities.cs`). An application this system builds lives under `Documents\PagentOS Projects
ative` — an authorised root, but not Program Files. The device track added the four build/package commands to that root and no launch path, so a freshly built EXE currently cannot be started by anything. Spec §5 already says the launch should go through `app.launch` on an authorised root path; it grants no new authority, since a compiler already runs under that root. **This is what blocks the "EXE launched" exit criterion even after item 28.** |
