...
## M24 — Capability Genesis (ADR-0087)

Gate: QUALIFICATION Stage 22 rows 22.1–22.8. Automated: `services/api` unit suite incl. `tests/unit/test_genesis_*.py` and the corpus category `genesis` (the fixture applications started by the harness); the web suite incl. `tests/uistate/genesis-states.test.ts` + `tests/cockpit/genesis-panel.test.tsx`. Real: a genesis run on production against a fixture application on the host. Merged main 6bac5e0 (web 5b85286, core 5f058a5, the review fixes 6bac5e0); CI run 34228908716 on d6bb9d5: 7/7 jobs green (API lint + unit, API integration, Windows agent build + tests, browser agent, web shell build, recovery supervisor, secret hygiene); green on every M24 commit before it as well (5f058a5 the core merge, 6bac5e0 the review fixes) and on 5b85286 the web merge.

## M25 — Unity / Blender / 3D creation (ADR-0088)

Gate: QUALIFICATION Stage 23 rows 23.1–23.10. Automated: `services/api` unit suite (6464 passed, 2 skipped) incl. `tests/unit/test_scene_*.py` and `test_blender_driver.py` and the corpus category `creative3d`; the agent project 758/758 incl. `tests/PagentOS.Agent.Tests/Scenes/`; the web suite 1318/1318 incl. `scenes.ts` and the "3B Sahne" panel. Real: the headless Blender round trip on this machine through the Cloud Core's own command (`scripts/tests/blender-service-command-lab.py`) as well as through the driver lab, and the Unity licence refusal as measured. Merged main 20db267 (the Cloud Core half bf791d5, the review fixes 007bb01); CI run 34254950816 on 20db267: 7/7 jobs green (API lint + unit, API integration, Windows agent build + tests, browser agent, web shell build, recovery supervisor, secret hygiene); green on 007bb01 (run 34247048976) as well — the runs on cb048b9, 41afaaf and 5e3e834 were superseded and cancelled by the next push.

## Owner Location Context, Live Weather, Morning Briefing (ADR-0091)

Gate: no dedicated QUALIFICATION stage assigned (this task had no milestone spec file; ADR-0091 is the spec). Automated: `services/api` unit suite additions — `test_location_service.py` (11, the owner-named resolution matrix), `test_weather_providers.py` (12, `OpenMeteoProvider` against `httpx.MockTransport`), `test_weather_service.py` (8), `test_briefing_service.py` (11), `test_voice_weather_briefing_intents.py` (24), `test_weather_briefing_wiring.py` (5), plus a regression in `test_actions_receipt.py` and tripwire updates in `test_voice_realtime_sessions.py`/`test_voice_intents.py` — and the voice-corpus category `weather` (107 cases: canonical, paraphrase, ASR-shaped, mandatory negatives), all passing through the real relay (`app.main.create_app`, never a handler called directly). Real: none — this branch is unmerged, unreleased and has no CI run of its own; live weather is proven only against a mocked HTTP transport (this sandbox has no outbound network), so the real Open-Meteo round trip is `NOT_YET_PROVEN` pending one run on the deployed Cloud Core. Not merged to main as of 2026-09-09 (branch `worktree-agent-a6616e449e5a5cf7e`).
## M26 addendum — Latest News Mode (ADR-0092)

**Status: NOT MERGED.** Built on `worktree-agent-ae92cb1700ac78996` (backend-engineer track), `services/api` and `services/browser` only — no web, no security review, no CI run on this branch, no owner qualification. Gate below is what THIS branch's own suites show; it is not a production acceptance.

Automated (`services/api`): the news-specific unit suite (`tests/unit/test_news_*.py`, 143 cases incl. `test_news_routes.py`'s REST surface) — the resolver's four fixture scenarios each checked against every content-policy value (`test_news_resolver.py`), Shorts/promo/bulletin-marker/near-duplicate classification (`test_news_classification.py`), channel-identity resolution never guessing from a display name (`test_news_identity.py`), the fixture and real YouTube-feed providers offline (`test_news_provider.py`), source CRUD and identity re-resolution (`test_news_sources_service.py`), the resolve/playback services incl. `TestCrossSessionPersistence` regressions for the `db.commit()` defect these tests themselves found (`test_news_resolve_service.py`, `test_news_playback_service.py`), and the ONE router's three new intents with capability/registry completeness (`test_news_voice_intents.py`). Voice corpus category `news` (`tests/voice_corpus/corpus.py::_news_cases`, 64 cases through the REAL relay against a deterministic fixture source/provider, `harness.py::CTX_NEWS_SOURCE_CONFIGURED`): every phrase in the task brief verbatim plus ASR-noise variants, the exact channel/video identity a receipt names, and the mandatory negatives (`news.open` never reaches `research.start`; `news.summarize` never reaches `news.open`; a bare "aç"/"özetle" with no "haber" noun stays outside the family) — full `tests/unit` suite (existing + these) green, no regressions in the wider corpus or the tool-schema/registry contract tests it touched (`test_voice_realtime_sessions.py`, `test_alarms_wiring.py::test_no_tool_schema_names_an_argument_the_relay_refuses`).

Automated (`services/browser`): `tests/unit/test_news_profile.py` (13 cases) — the third persistent `news` profile resolves to a distinct directory from both research and alarm, checked at worker startup; a news+alarm session pair coexist as separately-owned browsers; a second session on an already-owned `news` profile is refused (`browser_lifecycle_violation`); the media-op session-kind guard names `news` in its fix message. Full `services/browser` unit suite (404 cases) green, no regressions.

Real/live: ONE read-only qualification of `YouTubeFeedProvider` against NASA's real, public YouTube channel (`@pytest.mark.live`, not run by default CI) — 15 real candidates, real distinct `published_at` timestamps, `answered_by=channel_feed`. No governed-playback qualification against a real browser worker/device (none available in this environment); no qualification of the owner's own "Show Ana Haber" channel (deliberately left `needs_identity` — `docs/OWNER_ACTIONS.md` item 34).

Named gaps, stated plainly: the channel Videos-listing and DOM-extraction discovery tiers are unimplemented (both need a live governed browser session, unqualifiable offline); `news.close` has no deterministic voice phrase (not named in the task's own phrase list) and is untested by the corpus; no web/Cockpit surface; no security review.

## M26 — Executive Autonomy, with Owner Location Context, Live Weather, the Morning Briefing and Latest News Mode (ADR-0089, ADR-0091, ADR-0092)

Gate: QUALIFICATION Stage 24 rows 24.1–24.17. Automated: `services/api` unit suite **7312 passed / 2 skipped** (17m; `test_health_endpoint.py` excluded - it makes real db/redis/object-store/temporal connects and hangs in this sandbox, an environmental fault recorded in agent memory since 2026-09-02, not a regression); `services/browser` **405 passed**; `apps/web` **74 files / 1397 tests**, `tsc --noEmit` clean, `oxlint` 0 errors; `ruff check .` clean in both Python services; the voice corpus 1636 cases across 19 categories (`executive` 120 multi-turn, `weather` 107, `news` 64) through the ONE router. Reviews: three independent security reviews before the gate, ten findings, every one closed with a regression test verified to fail against the old code first. Real: M26 released to production blue/green (`e6f08ff`, `ui_state` 11); ONE executive run COMPLETED on production (`ec20975f`, 4/4 verified, document artifact `dd283204` read back READY); one live Open-Meteo answer with its evidence row; the briefing assembled from the live registries; the location resolver honestly unresolved with no default. Merged main e6f08ff; CI run 34301544141 green (all 7 jobs) on the released tree `e6f08ff`; the code-bearing run `34296886021` green before it.

## M27 — Creative Tools Operator (ADR-0093, ADR-0094)

Gate: QUALIFICATION Stage 25 rows 25.1–25.13. Automated: `services/api` unit suite **7570 passed / 3 skipped** (`tests/integration/*` is CI's own job — it needs real Postgres/Redis/MinIO/Temporal and cannot run in this sandbox); `services/browser` **412 passed**; `apps/web` **79 files / 1509 tests**, `tsc --noEmit` clean, `oxlint` 0 errors; `ruff check` clean; the voice corpus **1719 cases across 20 categories** (`creative` 83) through the ONE router; the creative unit suites **132** across ten files. Review: one independent security review before the gate, three findings, each closed with a regression verified to fail against the old code first — plus five defects found in OTHER milestones' code while building this one, each with its own regression. Real: the Paint lab on this machine against the real service with no fake in the path (a 3,703-byte PNG, reopened independently at 320×240, colours and 969 text ink pixels verified, a deliberate wrong colour caught as `missing_region` and corrected in one round); tool detection measured twice, with Photoshop's registry-key-without-an-executable named rather than imitated. Merged main `65ab820`; CI run `34321233803` green. Named gaps, stated rather than hidden: the M19 "opened in Paint" round trip on production and the device-side `creative.export_check` both wait on the elevated agent update (owner item 28), a boundary spec §6 named in advance; Photoshop / Illustrator / Figma are `PROVEN_PROXY` by construction because none is installed or configured here; narration audio stays gated on owner item 29.

## M28 — Native Application Factory (ADR-0095)

Gate: QUALIFICATION Stage 26 rows 26.1–26.17. Automated: `services/api` unit suite **7883 passed / 16 skipped** (CI run `34341441268`, all seven jobs green including the integration job); `services/browser` **412**; `apps/web` **81 files / 1576 tests**, `tsc` clean, `oxlint` clean; `devices/windows-agent` **973**; `ruff check .` clean in both Python services; the voice corpus **1813 cases / 21 categories** (`nativeapps` 94) with every earlier category unchanged; M28's own **117** unit tests across eight files plus **78** voice tests across two. Real: an owner-shaped request became a **162,304-byte `notlarim.exe`** (x64, `windows_gui`, version read back as `0.1.0`, sha256 `175164a9…`, verdict AGREES), a **59.5 MB** portable package and a **60.6 MB MSIX** built by Windows' own `makeappx` and read back as `PagentOS.notlarim` / `0.1.0.0` / `Notlarım` / 250 entries — each opened by a reader that did not build it, and the reader itself qualified against binaries it did not build, including this repository's own installed agent. The build is reproducible: byte-identical sha256 across runs. Merged main `7527877`. Named gaps, stated rather than omitted: the EXE has NOT been launched (26.15 — owner item 28, and `qualify-item28-unlocked.ps1` closes it automatically), a build is not startable from production (26.16), Android is `WAITING_OWNER_JDK` (item 33), iOS is `NOT_BUILT` and is not a target value, and narration audio is `PROVIDER_UNAVAILABLE` (item 29).
